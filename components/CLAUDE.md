...
6. **Coach "Move to" is a non-atomic DELETE-then-POST.** `components/main/coach/club/coach-row.tsx:51-82` removes the coach from the source group, then adds to the target; if the POST fails (network drop, seat limit) the coach is left in neither group with no compensating re-add. Fix: atomic move endpoint, or re-add on failure.
...
12. **Dead code:** `app/sentry-example-page/page.tsx` is orphaned (only self-references) and ships a "Test Sentry" page to production. Pairs with `app/api/sentry-example-api/route.ts` (API-audit finding 25) — remove both together.
